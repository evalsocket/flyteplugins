...
func dummyPytorchCustomObj(workers int32) *plugins.DistributedMPITrainingTask {
...
func dummySparkTaskTemplate(id string, mpiCustomObj *plugins.DistributedMPITrainingTask) *core.TaskTemplate {
	...
	ptObjJSON, err := utils.MarshalToString(mpiCustomObj)
	if err != nil {
		panic(err)
	}
...
func dummyPytorchJobResource(pytorchResourceHandler mpiOperatorResourceHandler, workers int32, conditionType commonOp.JobConditionType) *mpiOp.MPIJob {
	...
	return &mpiOp.MPIJob{
		...
		Spec: resource.(*mpiOp.MPIJob).Spec,
...
func TestBuildResourcePytorch(t *testing.T) {
	pytorchResourceHandler := mpiOperatorResourceHandler{}
	...
	pytorchJob, ok := resource.(*mpiOp.MPIJob)
	assert.True(t, ok)
	assert.Equal(t, int32(100), *pytorchJob.Spec.MPIReplicaSpecs[mpiOp.MPIReplicaTypeWorker].Replicas)

	for _, replicaSpec := range pytorchJob.Spec.MPIReplicaSpecs {
		var hasContainerWithDefaultPytorchName = false

		for _, container := range replicaSpec.Template.Spec.Containers {
			if container.Name == mpiOp.DefaultContainerName {
...
func TestGetTaskPhase(t *testing.T) {
	...
	dummyPytorchJobResourceCreator := func(conditionType commonOp.JobConditionType) *mpiOp.MPIJob {
